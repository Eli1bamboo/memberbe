...
class UserController {
  ...
  async update(req, res) {
    try {
      let user = await UserManager.getByEmail(req.query.email);
      if (user) {
        if (req.query.customerId === '') delete req.query.customerId
        user = await UserManager.update(req.query, req.params.userId)
        user = await UserManager.getByEmail(req.query.email);
        delete user._doc.passwordHash;
        return res.status(200).send({
          status: 'updated',
          user,
