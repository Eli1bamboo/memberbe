...
class UserManager {
    ...
    async login(email, password) {
        ...
        var confirmed=false;
        try{confirmed = await bcrypt.compare(password, user.passwordHash);}
        catch(err){};
        if (confirmed){
            delete user._doc.passwordHash;
            var token = jwt.sign({email: user.email, roles: user.roles}, SECRET);
    ...
    async sendCreatePasswordMail(user){
        ...
        sendMail('Create Your New Password At Membrify', 
    ...
    async changePassword(user, data){
        ...
        if (confirmed){
            ...
            return await user.save().then(function (user){
                sendMail('Password Changed Membrify', 
    ...
    async resetPasswordStepOne(user){
        ...
        var token = jwt.sign({email:user.email}, SECRET+'resetPass', { expiresIn: '4h' })
        const frontHost = process.env.FRONT_HOST + `retrieve-password-step-two?email=${user.email}&token=${token}`;
        sendMail('Retreive Password Membrify', 
    ...
    async resetPasswordStepTwo(req, res){
        ...
        */
        if (!req.query.token)
            return res.status(400).send({status: 'ok',message: 'Not token provided'});
        if (!req.query.newPassword)
            return res.status(400).send({status: 'ok',message: 'Not newPassword provided'});
        var decoded='';
        try { decoded = jwt.verify(req.query.token, SECRET+'resetPass'); } 
        catch (error) {
        ...
        if (decoded){
            var user = await this.getByEmail(decoded.email)
            if (!user)
                return res.status(400).send({status: 'ok',message: 'Invalid Token user, not found',error: error});
            ...
            return await user.save().then(function (user){
                sendMail('Password Changed', 
                    ...
                    user.email)
                delete user._doc.passwordHash;
                var newToken = jwt.sign({email: user.email, roles: user.roles}, SECRET);
                user.token = newToken;
